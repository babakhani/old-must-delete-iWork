...
Public Class AccountController
    ...
    Private _repo As IAccountRepository = Nothing

    Public Sub New()
        _repo = Application.GetService(Of IAccountRepository)()
    End Sub

    Public Function Register(userModel As UserModel) As IHttpActionResult

        If Not ModelState.IsValid Then
            Return BadRequest(ModelState)
        End If

        Dim result As IdentityResult = _repo.RegisterUser(userModel.UserName, userModel.Password).Result
...
'        'Private Function GetService() As IAccountService
'        '    Return Application.GetService(Of IAccountService)()
'        'End Function
...
'        'Public Function Register(user As User) As IHttpActionResult

'        '    If Not ModelState.IsValid Then
'        '        Return BadRequest(ModelState)
'        '    End If

'        '    Dim result = GetService.Register(user)

'        '    If GetService.Register(user) Then
'        '        Return Ok()
'        '    Else
'        '        Return BadRequest("Register Exception")
'        '    End If
...
'        '    Dim user = GetRepository.FindAsync(New UserLoginInfo(externalLogin.LoginProvider, externalLogin.ProviderKey)).Result
...
'        '    Dim user = Await GetRepository.FindAsync(New UserLoginInfo(model.Provider, verifiedAccessToken.user_id))
...
'        '    Dim result = Await GetRepository.CreateAsync(user)
'        '    If Not result.Succeeded Then
...
'        '    'result = Await GetService.Register(user.Id, info.Login)
'        '    'If Not result.Succeeded Then
'        '    '    Return GetErrorResult(result)
'        '    'End If
...
'        '    Dim user = Await GetRepository.FindAsync(New UserLoginInfo(provider, verifiedAccessToken.user_id))
...
'        '    Dim client = GetRepository.FindClient(clientId)
